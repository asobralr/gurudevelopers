...
After downloading the file from Themeforest, You will find SuperProps.zip file along with Documentation.pdf file. Then unzip the SuperProps.zip and run the following commands on SuperProps folder to get started with the project.
...
```
/packages
	/landing [All the source codes are available here]
```
...
From `/common/data/` , you will find all the datas used for the templates. Each folder contains `index.js` and `index.json` file. We have used `index.js` file for the templates . We have also `index.json` file for our customer if they want to use it.
